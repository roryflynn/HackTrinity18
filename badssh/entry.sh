...
DAEMON=sshd
SFTP_MODE='true'
SFTP_CHROOT='/chroothome'
SSH_USERS='badssh:1011:1011'
MOTD='flag'
mkdir -p /data
...
if [[ "${SFTP_MODE}" == "true" ]]; then
    ...
    echo 'setting sftp chroot'
    mkdir -p /chroothome/{dev,etc,lib,usr,bin}
    # mkdir -p /chroothome/usr/bin
    # mknod -m 666 /chroothome/dev/null c 1 3

    chown root:badssh /chroothome/bin/sh

    ln -s /chroothome/bin/sh /chroothome/bin/bash

    chmod 0510 /chroothome/bin/bash
    ...
    printf '%s\n' \
        ...
        'set /files/etc/ssh/sshd_config/ForceCommand /printflag' \
        'set /files/etc/ssh/sshd_config/ChrootDirectory /chroothome' \
